      ...
      if (renderToolBox) {
        column.renderBody = (h, row) => {
          ...
          if (templates.length > this.toolboxSize) {
            ...
            let dropdownItem = vNodeHide.map((node) => {
              return this.getRenderDropdownItem(node)
            })
            // get dropdown link's vNodes
            let dropdownLink = this.getRenderDropdownLink(column.tooltip)
            // get dropdown's vNodes
            let dropdowns = this.getRenderDropdown(dropdownLink, dropdownItem)
